...
export class EditListComponent implements OnInit {
  ...
  handleForm() {
    this.todoForm = this.formService.group({
      ...
      description: [this.currentList.description, [Validators.required]],
  ...
  saveForm() {
    if (this.todoForm.valid.valueOf()) {
      this.currentList = { ...this.currentList, ...this.todoForm.value };

      if (this.isNewList) {
        this.listService.addNewList(this.currentList);
      } else {
        this.listService.editList(this.currentList);
      }
